...
export const UPLOAD_TARGET_STORAGE_KEY = "carvioo_upload_target";
...
export const getUploadTarget = () => {
  ...
  if (stored === UPLOAD_TARGETS.VPS) return UPLOAD_TARGETS.VPS;
  if (stored === UPLOAD_TARGETS.CLOUDINARY) return UPLOAD_TARGETS.CLOUDINARY;
  return UPLOAD_TARGETS.DEFAULT;
};

export const setUploadTarget = (target) => {
  if (typeof window === "undefined") return UPLOAD_TARGETS.DEFAULT;
  const validTargets = Object.values(UPLOAD_TARGETS);
  const nextTarget = validTargets.includes(target) ? target : UPLOAD_TARGETS.DEFAULT;
...
export const getUploadAuthHeaders = () => {
  ...
  const headers = { "x-upload-target": target };
  if (token) {
