      ...
      if (item.type === "section") {
        ...
        item.items?.forEach((suCarvioom) => {
          if (suCarvioom.type === "expandable") {
            // Include main parent
            labels.push({ type: "item", label: suCarvioom.label });
            // Include sub items for granular permissions
            suCarvioom.suCarviooms?.forEach((deepSuCarvioom) => {
              labels.push({ type: "sub-item", label: deepSuCarvioom.label, parent: suCarvioom.label });
            });
          } else {
            labels.push({ type: "item", label: suCarvioom.label });
          }
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2">
                {menuItems.map((item, index) => {
                  ...
                  const isSuCarvioom = item.type === "sub-item";

                  return (
                    <div
                      key={index}
                      className={`flex items-center justify-between p-3 rounded-lg hover:bg-gray-50 border border-transparent hover:border-gray-100 transition-colors cursor-pointer ${isSuCarvioom ? "ml-8 border-l-2 border-l-gray-200 rounded-l-none" : ""
                        }`}
                      onClick={() => handleToggleModule(item.label)}
                    >
                      <span className={`${isSuCarvioom ? "text-gray-600 text-sm" : "text-gray-700 font-medium"}`}>
                        {isSuCarvioom && <span className="text-gray-300 mr-2">↳</span>}
                        {item.label}
                      </span>
                      <div className="relative inline-flex items-center">
                        <input
                          ...
                          readOnly
                        />
                        <div className={`w-9 h-5 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all ${isSuCarvioom ? "peer-checked:bg-teal-500 scale-90" : "peer-checked:bg-[#11b5b8]"}`}></div>
                      </div>
                    </div>
                  );
                })}
