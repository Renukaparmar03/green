...
export default function ToggleManagement() {
  ...
  const fetchBusinessSettings = async () => {
    try {
      ...
      if (settings) {
        ...
        if (settings.uploadDestination) {
          const valid = [UPLOAD_TARGETS.DEFAULT, UPLOAD_TARGETS.VPS, UPLOAD_TARGETS.CLOUDINARY];
          if (valid.includes(settings.uploadDestination)) {
            setUploadTarget(settings.uploadDestination);
            setUploadTargetState(settings.uploadDestination);
          }
        }
      }
    } catch (error) {
  ...
  const handleSelectUploadTarget = async (target) => {
    if (uploadToggleBusy) return;

    if (target === UPLOAD_TARGETS.VPS && !vpsUploadReady) {
    ...
    try {
      ...
      const savedTarget = setUploadTarget(target);
      setUploadTargetState(savedTarget);
      await persistToggles({ uploadDestination: savedTarget }, "uploadDestination");

      let message = "Uploads will now go to Local storage";
      if (savedTarget === UPLOAD_TARGETS.VPS) {
        message = "Uploads will now go to the VPS backend";
      } else if (savedTarget === UPLOAD_TARGETS.CLOUDINARY) {
        message = "Uploads will now go to Cloudinary CDN";
      }
        ...
        <div className="bg-white rounded-lg shadow-sm border border-slate-200">
          <div className="px-4 py-4 border-b border-slate-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="md:col-span-2 flex flex-col sm:flex-row sm:items-center justify-between border border-slate-100 p-4 rounded-xl bg-slate-50/50 gap-4">
                <div>
                  <p className="text-sm font-semibold text-slate-800">Upload Destination</p>
                  <p className="text-xs text-slate-500 mt-0.5">Select where newly uploaded images, documents, and videos should be stored and served from.</p>
                  <p className="text-[11px] text-slate-400 mt-2 break-all">
                    Active upload mode: <span className="font-bold text-blue-600 uppercase">{uploadTarget}</span> ({uploadBaseUrl})
                  </p>
                  {!vpsUploadReady && uploadTarget === UPLOAD_TARGETS.VPS && (
                    <p className="text-[11px] text-amber-700 mt-2">
                      VPS is selected but <span className="font-semibold">VITE_VPS_UPLOAD_API_BASE_URL</span> is not set.
                    </p>
                  )}
                </div>
                <div className="flex items-center gap-1.5 bg-slate-200/70 p-1 rounded-xl flex-shrink-0 self-start sm:self-auto">
                  <button
                    type="button"
                    disabled={uploadToggleBusy}
                    onClick={() => handleSelectUploadTarget(UPLOAD_TARGETS.DEFAULT)}
                    className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all ${
                      uploadTarget === UPLOAD_TARGETS.DEFAULT
                  ...
                  <button
                    type="button"
                    disabled={uploadToggleBusy}
                    onClick={() => handleSelectUploadTarget(UPLOAD_TARGETS.VPS)}
                    className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all ${
                      uploadTarget === UPLOAD_TARGETS.VPS
                        ? "bg-white text-blue-600 shadow-sm"
                        : "text-slate-600 hover:text-slate-900"
                    }`}
                  >
                  ...
                  <button
                    type="button"
                    disabled={uploadToggleBusy}
                    onClick={() => handleSelectUploadTarget(UPLOAD_TARGETS.CLOUDINARY)}
                    className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all ${
                      uploadTarget === UPLOAD_TARGETS.CLOUDINARY
                  ...
                  {uploadToggleBusy && <Loader2 className="w-4 h-4 animate-spin text-blue-600 ml-1" />}
                </div>
              </div>
