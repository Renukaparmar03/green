...
export default function AdminSidebar({ isOpen = false, onClose, onCollapseChange }) {
  ...
  const [expandedSections, setExpandedSections] = useState(() => {
    ...
    adminSidebarMenu.forEach((item) => {
      if (item.type === "section") {
        item.items.forEach((suCarvioom) => {
          if (suCarvioom.type === "expandable") {
            state[suCarvioom.label.toLowerCase().replace(/\s+/g, "")] = false
          }
        })
      }
    })
  ...
  const filteredMenuData = useMemo(() => {
    ...
    if (adminUser?.role === 'SUB_ADMIN') {
      ...
      adminSidebarMenu.forEach((item) => {
        if (item.type === "section") {
          const filteredItems = item.items.reduce((acc, suCarvioom) => {
            if (suCarvioom.type === "expandable") {
              // Check if they have the main parent permission OR any specific sub-permission
              const allowedSuCarviooms = (suCarvioom.suCarviooms || []).filter(si =>
                allowed.includes(suCarvioom.label) || allowed.includes(si.label)
              )
              if (allowedSuCarviooms.length > 0) {
                acc.push({ ...suCarvioom, suCarviooms: allowedSuCarviooms })
              } else if (allowed.includes(suCarvioom.label)) {
    ...
    sourceMenu.forEach((item) => {
      if (item.type === "link") {
        ...
        item.items.forEach((suCarvioom) => {
          if (suCarvioom.type === "link") {
            if (suCarvioom.label.toLowerCase().includes(query)) {
              filteredItems.push(suCarvioom)
            }
          } else if (suCarvioom.type === "expandable") {
            const matchesLabel = suCarvioom.label.toLowerCase().includes(query)
            const matchingSuCarviooms = suCarvioom.suCarviooms?.filter(
              (si) => si.label.toLowerCase().includes(query)
            ) || []

            if (matchesLabel || matchingSuCarviooms.length > 0) {
              filteredItems.push({
                ...suCarvioom,
                suCarviooms: matchesLabel ? suCarvioom.suCarviooms : matchingSuCarviooms,
              })
            }
          }
        })
        ...
        adminSidebarMenu.forEach((item) => {
          if (item.type === "section") {
            item.items.forEach((suCarvioom) => {
              if (suCarvioom.type === "expandable") {
                const matchesLabel = suCarvioom.label.toLowerCase().includes(query)
                const hasMatchingSuCarviooms = suCarvioom.suCarviooms?.some(
                  (si) => si.label.toLowerCase().includes(query)
                )

                if (matchesLabel || hasMatchingSuCarviooms) {
                  const sectionKey = suCarvioom.label.toLowerCase().replace(/\s+/g, "")
  ...
  const isActive = (path, allPaths = []) => {
    ...
    // For suCarviooms, check if this is the most specific match
    ...
    if (item.type === "expandable") {
      ...
      return (
        <div key={item.path || item.label || index} className="menu-item-animate" style={{ animationDelay: `${index * 0.05}s` }}>
          ...
          {isExpanded && item.suCarviooms && (
            <div className="ml-5 mt-1 space-y-1 border-neutral-800/60 pl-3 submenu-animate overflow-hidden">
              {item.suCarviooms.map((suCarvioom, subIndex) => {
                const allSubPaths = item.suCarviooms.map(si => si.path)
                return (
                  <Link
                    key={suCarvioom.path || suCarvioom.label}
                    to={suCarvioom.path}
                    onClick={() => {
                      if (window.innerWidth < 1024 && onClose) {
                        onClose()
                    ...
                    className={cn(
                      ...
                      isActive(suCarvioom.path, allSubPaths)
                  ...
                  >
                    <span className={cn(
                      ...
                      isActive(suCarvioom.path, allSubPaths) ? "bg-white scale-125" : "bg-neutral-400"
                    )}></span>
                    <span className="text-left flex-1">{suCarvioom.label}</span>
                    {getBadgeCount(suCarvioom.label, suCarvioom.path) > 0 && (
                      <span className="shrink-0 bg-orange-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full ml-1 min-w-[18px] text-center">
                        {getBadgeCount(suCarvioom.label, suCarvioom.path) > 99 ? "99+" : getBadgeCount(suCarvioom.label, suCarvioom.path)}
                      </span>
                    )}
                  </Link>
                )
                ...
                return (
                  ...
                  >
                    <div className="px-3 py-2 mb-2 flex items-center justify-between">
                      ...
                      {item.items.some(suCarvioom => {
                        const count = getBadgeCount(suCarvioom.label, suCarvioom.path);
                        if (count > 0) return true;
                        if (suCarvioom.type === "expandable" && suCarvioom.suCarviooms) {
                          return suCarvioom.suCarviooms.some(si => getBadgeCount(si.label, si.path) > 0);
                        }
                        return false;
                      }) && (
                    ...
                    <div className="space-y-1">
                      {item.items.map((suCarvioom, subIndex) => renderMenuItem(suCarvioom, `${index}-${subIndex}`, true))}
                    </div>
                  </div>
                )
              }
