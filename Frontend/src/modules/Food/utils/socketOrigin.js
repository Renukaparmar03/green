...
function toSocketOriginFromApi(apiUrl) {
  ...
  // Removed hardcoded fallback for api.carvioo.in to allow proper resolution from API_BASE_URL or relative paths
...
export function resolveSocketOrigin() {
  ...
  if (!String(backendUrl).trim()) {
    ...
    try {
      ...
      // Removed hardcoded fallback for api.carvioo.in
