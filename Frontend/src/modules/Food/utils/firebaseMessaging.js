...
function getDeviceId(moduleName, devicePlatform, fcmToken, voipToken) {
  ...
  return ["carvioo", moduleName, devicePlatform || "unknown", tokenSeed || "device"].join(":");
}
...
export async function registerWebPushForCurrentModule(pathname = window.location.pathname, options = {}) {
  ...
  if (!nativeToken && options?.throwOnError === true) {
    throw new Error("Push notifications are not supported in this browser mode. On iPhone, install Carvioo using Add to Home Screen first.");
  }
  return nativeToken;
}
