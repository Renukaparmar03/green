...
export const uploadToCloudinary = async ({ buffer, prefix = 'upload', folder = 'uploads', resourceType = 'auto' }) => {
    if (!isCloudinaryConfigured()) {
        throw new Error('Cloudinary credentials are not configured in environment variables (.env)');
    }

    return new Promise((resolve, reject) => {
        const publicId = `${normalizeUploadToken(prefix, 'upload')}_${uuidv4().replace(/-/g, '').substring(0, 10)}`;
...
/**
 * Processes and saves an image buffer to the single upload directory or Cloudinary.
 * Returns the relative public path or full Cloudinary URL
 */
const processAndSaveImage = async ({ buffer, prefix, folder = 'uploads', width, height, quality = 80, target }) => {
    if (target === 'cloudinary') {
        return uploadToCloudinary({ buffer, prefix, folder, resourceType: 'image' });
    }
...
export const uploadFoodImage = async (buffer, options = {}) => {
...
export const uploadRestaurantImage = async (buffer, options = {}) => {
    return processAndSaveImage({
        buffer,
        folder: 'restaurants',
        prefix: 'restaurant',
...
export const uploadBannerImage = async (buffer, options = {}) => {
    return processAndSaveImage({
        buffer,
        folder: 'banners',
        prefix: 'banner',
...
export const uploadDeliveryImage = async (buffer, options = {}) => {
    return processAndSaveImage({
        buffer,
        folder: 'delivery',
        prefix: 'delivery',
...
export const uploadGenericImage = async (buffer, _folder = 'misc', options = {}) => {
...
export const uploadFileBuffer = async (buffer, _folder = 'misc', options = {}) => {
    if (options?.target === 'cloudinary') {
        return uploadToCloudinary({ buffer, prefix: 'file', folder: _folder, resourceType: 'raw' });
    }
    const dir = ensureUploadDirExists();
...
export const uploadVideoBuffer = async (buffer, _folder = 'videos', options = {}) => {
    if (options?.target === 'cloudinary') {
        return uploadToCloudinary({ buffer, prefix: 'video', folder: _folder, resourceType: 'video' });
    }
    const dir = ensureUploadDirExists();
    const filename = buildFlatUploadFilename({
        prefix: 'video',
