...
const getUploadTarget = (req) => {
    return req.headers['x-upload-target'] || req.body?.target || req.query?.target || 'default';
};
...
router.post('/image', upload.single('file'), async (req, res, next) => {
    try {
        ...
        const url = await uploadGenericImage(req.file.buffer, folderLabel, { target });
...
router.post('/video', upload.single('file'), async (req, res, next) => {
    try {
        ...
        const target = getUploadTarget(req);
        const url = await uploadVideoBuffer(req.file.buffer, folderLabel, { target });
