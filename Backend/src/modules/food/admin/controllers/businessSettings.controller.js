...
export async function updateBusinessToggles(req, res, next) {
    try {
        ...
        if (uploadDestination !== undefined) {
            const valid = ['default', 'vps', 'cloudinary'];
            if (valid.includes(String(uploadDestination).toLowerCase())) {
                settings.uploadDestination = String(uploadDestination).toLowerCase();
            }
        }
