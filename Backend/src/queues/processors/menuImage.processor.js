...
export function buildImagePrompt(itemName, itemDescription, categoryName, foodType) {
    ...
    let prompt = `A professional, highly realistic, appetizing food photography image of the exact Carvioo Food dish: "${dishName}".`;

    if (desc) prompt += ` The dish is described as: "${desc}".`;
    if (category) prompt += ` It belongs to the "${category}" category on an Carvioo Food restaurant menu.`;

    prompt += ` This is a ${type} Carvioo Food dish.`;
    prompt += ` The photo must show ONLY this specific dish — "${dishName}" — served in a traditional or modern Carvioo Food restaurant style.`;
