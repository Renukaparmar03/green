...
async function run() {
    try {
        const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/Carvioo';
        ...
        await Restaurant.deleteMany({ ownerName: 'Carvioo Demo' });
        await DeliveryPartner.deleteMany({ name: { $regex: 'Carvioo Rider' } });
        await HeroBanner.deleteMany({ publicId: { $regex: 'demo_banner_' } });

        await FoodItem.deleteMany({ name: { $regex: 'Carvioo' } }); // Clear previous demo items if any
